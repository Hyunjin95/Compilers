...
hello:
    # stack offsets:
    #      8(%ebp)   4  [ %b        <int> %ebp+8 ]
    #     24(%ebp)   1  [ %e        <char> %ebp+24 ]
    #    -13(%ebp)   1  [ $i        <char> %ebp-13 ]
    #     16(%ebp)   1  [ %q        <char> %ebp+16 ]
    #     28(%ebp)   1  [ %r        <char> %ebp+28 ]
    #     12(%ebp)   1  [ %t        <bool> %ebp+12 ]
    #     20(%ebp)   1  [ %w        <char> %ebp+20 ]
    ...
    # function body
    movl    $99, %eax               #   0:     assign i <- 99
    movb    %al, -13(%ebp)         
    jmp     l_hello_0               #   1:     goto   0
    movl    $100, %eax              #   2:     assign i <- 100
    movb    %al, -13(%ebp)         
l_hello_0:
...
main:
    ...
    # function body
    leal    arr, %eax               #   0:     &()    t0 <- arr
    movl    %eax, -16(%ebp)        
    movl    $2, %eax                #   1:     param  1 <- 2
    pushl   %eax                   
    leal    arr, %eax               #   2:     &()    t1 <- arr
    movl    %eax, -20(%ebp)        
